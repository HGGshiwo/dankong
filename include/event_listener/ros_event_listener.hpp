...
// 和ROS相关的操作的事件走这个监听器
class RosEventListener
    : public dk::BaseEventListener<RobotContext, RosEventListener> {
   private:
    ros::NodeHandle nh_;
   ...
   public:
    ...
    RosEventListener() : nh_() {
        ...
        stop_record_client_ =
            std::make_shared<ServiceClient<std_srvs::Trigger>>(
    ...
    void on_event(const GetGimbalEvent& event, RobotContext& ctx) {
        ...
        nh_.param<std::string>("/UAV0/sensor/serial_gimbal/angle_mode", mode,
                               "");
        nh_.param<double>("/UAV0/sensor/serial_gimbal/gimbal_angle", angle,
                          0.0);
    ...
    void on_event(const GetExposureEvent& event, RobotContext& ctx) {
        ...
        nh_.param<double>("/UAV0/sensor/video11_camera/shutter_max",
                          shutter_max, 100.0);
        nh_.param<double>("/UAV0/sensor/video11_camera/", shutter_min, 0.0);
        nh_.param<double>("/UAV0/sensor/video11_camera/", shutter_step, 1.0);
        nh_.param<double>("/UAV0/sensor/video11_camera/ISO", sens, 50.0);
        nh_.param<double>("/UAV0/sensor/video11_camera/ISO_max", sens_max,
                          100.0);
        nh_.param<double>("/UAV0/sensor/video11_camera/ISO_min", sens_min, 0.0);
        nh_.param<double>("/UAV0/sensor/video11_camera/ISO_step", sens_step,
                          2.0);
