...
class ReportFeature {
   public:
    ...
    //     { auto deviceCode = GlobalConfig.GetConfig().device_code.get(); if
    //     (!deviceCode.has_value()) return;
    ...
    //         spdlog::info("[Mqtt] use existing code: {}",
    //         device_code.value()); mqtt->connect(device_code.value());
