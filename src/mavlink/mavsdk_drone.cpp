...
bool MavsdkDrone::set_mode(const mavsdk::Telemetry::FlightMode& mode) {
    // 优先处理 MAVSDK 封装好的标准动作
    if (mode == mavsdk::Telemetry::FlightMode::ReturnToLaunch) {
        return action_->return_to_launch() == mavsdk::Action::Result::Success;
    } else if (mode == mavsdk::Telemetry::FlightMode::Land) {
        // 注：Rover 通常不支持
    ...
    // 查找并下发 Custom Mode
    float custom_mode = get_mode_param_(mode);
    ...
    if (result != mavsdk::MavlinkPassthrough::Result::Success) {
        spdlog::error("Failed to set APM mode {} (Code {})",
                      FlightMode::mode_raw_to_string(mode), custom_mode);
        return false;
    }
    spdlog::info("Successfully set APM mode to {}",
                 FlightMode::mode_raw_to_string(mode));
    return true;
}

bool MavsdkDrone::run_prearm_checks() {
    set_mode(mavsdk::Telemetry::FlightMode::Offboard);
    if (!passthrough_) {
